...
function simulate(nodes, links) {
    let rectangleCollide = d3.bboxCollide((d) => {
        let box = [ 
                    ...
                    // bottom right corner
                    [d.width + terrapinViewConfig.connectorRadius, d.height + terrapinViewConfig.connectorRadius]
                ];
        return box;
    });

    let attractForce = d3.forceManyBody().strength(200).distanceMax(700).distanceMin(300);
    let repelForce = d3.forceManyBody().strength(-140).distanceMax(700).distanceMin(300);

    simulation = d3.forceSimulation()
                    // .force("charge", d3.forceManyBody().strength(() => {return -1}))
                    .force('link', d3.forceLink().id((d) => { return d.id }).distance(0).strength(0)) // distance(50).strength(1))
...
function dragEnded(d) {
    ...
    d.fx = null;
    d.fy = null;
    if (terrapinViewConfig.snapTogrid) {
        d.x = snapTogrid(d.x);
        d.y = snapTogrid(d.y);
    }
    d3.select(this)
...
function zoomToElement(element) {
    var d = element.datum();
    var scale = terrapinViewConfig.zoomOnElementFocus,
        x = d.x + d.width / 2,
        y = d.y + d.height / 2;
...
function snapTogrid (coord) {
    return Math.round(coord / terrapinViewConfig.gridStep) * terrapinViewConfig.gridStep;
}
